...
export const getLectures = functions.https.onRequest(
  async (request, response) => {
    ...
    const possibleLecturesURLs = possibleLecturesPaths.map((e) =>
      urljoin(courseObj.data.url, e)
    );
      ...
      if (possibleLecturesFeatureNames.includes(feature.name)) {
        if (isLocalURL(feature.url)) {
          if (possibleLecturesURLs.includes(feature.url)) {
            possibleLecturesURLs.slice(
              possibleLecturesURLs.indexOf(feature.url),
              1
            );
          } else {
    ...
    for (const lecturesURL of possibleLecturesURLs) {
      ...
      lectureElements.forEach((lectureElement: any, lectureIndex: number) => {
        const url =
          lectureElement.querySelector("a")?.getAttribute("href") || "";
        const thumbnailURL =
          lectureElement.querySelector("img")?.getAttribute("src") || "";
        const title = removeUselessWhitespace(
          lectureElement.querySelector(".mediatitle")?.textContent || ""
        );
          ...
          const lectureObj = new MinimalLecture({
            ...
            thumbnailURL:
              thumbnailURL.length > 0
                ? parseURL(
                    thumbnailURL.replace("/default.jpg", "/sddefault.jpg")
                  )
                : "",
            title,
            lectureIndex,
