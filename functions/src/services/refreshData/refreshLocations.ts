...
const refreshLocations = async () => {
  ...
  for (const course of courses.docs) {
    ...
    for (location of courseLocations) {
      ...
      if (!locationsToCourseIDsMap[locationAsJSONString])
        locationsToCourseIDsMap[locationAsJSONString] = [];
      locationsToCourseIDsMap[locationAsJSONString].push(courseObj.id);
    }
  }

  // cache locations map
  const locationsAsObjs = removeDuplicatesFromArray(locationsAsJSONStrings).map(
  ...
  locationsAsObjs.forEach((location: Course["data"]["locations"][0]) => {
    ...
    if (location.topic && !locationsMap[location.topic])
      locationsMap[location.topic] = {};
    if (location.category && !locationsMap[location.topic][location.category])
      locationsMap[location.topic][location.category] = {};
    if (
      location.speciality &&
  ...
  const convertLocationsHasOwnItemsKeyToBooleanVal = (obj: GenericObject) => {
    Object.keys(obj).forEach((k) => {
      if (
        obj[k][LOCATIONS_HAS_OWN_ITEMS_KEY] === true &&
        Object.keys(obj[k]).length === 1
      ) {
        obj[k] = true;
      } else {
